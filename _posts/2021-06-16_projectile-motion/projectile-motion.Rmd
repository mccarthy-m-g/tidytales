...
I think cutting the bounce example is a good idea
...
Now for theming. I wanted something minimalistic with a scientific feel---the Tufte theme paired with `geom_rangeframe()` from ggthemes does this nicely. To correctly draw `geom_rangeframe()` I've set `clip` to off in `coord_cartesian()`, as the function's documentation recommends. Finally, `element_markdown()` from ggtext is being used to enable markdown text in in the title and subtitle in the plot. I'm using this to get subscripts and line breaks.

<aside>
I also tried base R methods to get line breaks and subscripts but they did not work along with gganimate. I like the ggtext syntax better anyways, but felt it was worth noting this.
</aside>
...
```{r}
#TODO: Generate random x, y, and size values that can be used for stars in the background. could also add a time dimension to make them twinkle. can use the diamond shape to make them look starlike
# the shadowtext package or whatever it was could be used to add a glow effect to the propulsion or stars

# Make it so the propulsion is only present for first half of animation
planets_shadow <- planets_trajectory %>%
  group_by(planet) %>%
...
star_background <- tibble(
  x = rep(x, 100),
  y = rep(y, 100),
  size = rep(sample(c(1, 1.5), 150, prob = c(0.75, 0.5), replace = TRUE), 100),
  alpha = rep(rnorm(150, 0.5, 0.5), 100),
  second = rep(test_data$second, each = 150)
)

ggplot(mapping = aes(x, y, group = 1)) +
  geom_point(
    data = star_background,
    aes(size = size, alpha = alpha),
    shape = 23,
    colour = "white",
    fill = "white"
  ) +
  ...
  transition_states(second)
```


```{r}
test_data <- planets_trajectory %>%
  filter(planet == "earth" | planet == "mars")

test_data %>%
  group_by(planet) %>%
  summarise(end = tail(second, 1))

p <- test_data %>%
  mutate(second = case_when(planet == "earth" ~ second,
                            planet == "mars" ~ second + 4.48)
  ) %>%
  ggplot(aes(x = x, y = y)) +
  geom_line(
...
ggplot(aes(x = x, y = y)) +
  ...
  geom_text(
    aes(colour = planet, group = planet, label = emojifont::fontawesome("fa-rocket")),
...
# TODO: This is a good size for sharing on Twitter. I should note this down in the wiki
animate(anim,
        nframes = 100,
        device = "ragg_png",
        width = 960,
