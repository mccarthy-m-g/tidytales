...
ggplot(demons_souls_plot) +
  ...
  # text.
  # FIXME: alpha isn't working.
  geom_segment(aes(x = 0, xend = 0, y = 10.9, yend = 0, size = 0.6), data = data.frame(),
               lineend = "round", colour = "grey", alpha = 0.3, linetype = "twodash") +
  ...
  # Draw caption
  annotate("text", x = 1, y = 10.33, angle = 270, hjust = 0, size = 3, alpha = 0.3,
           label = "SOURCE: PLAYSTATION NETWORK | GRAPHIC: MICHAEL MCCARTHY",  family = optimus, color = "grey") +
...
mask <- image_read(here("_posts", "2021-06-09_demons-souls", "figures", "texture.png")) %>%
  image_transparent("white") %>%
  image_threshold("black", "90%")

image_composite(image_read(file),
                mask,
                operator = "Over", #"DstOut" "Over
                offset = "+0+0")
```
