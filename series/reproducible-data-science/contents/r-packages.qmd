...
```{r}
#| label: cran-pkgsearch
#| echo: false
#| cache: true
library(pkgsearch)

# Packages *currently available* on CRAN
cran_pkgs <- as.data.frame(available.packages())

pkgs <- lapply(
  cran_pkgs$Package,
  function(x) {
    pkg <- pkgsearch::cran_package_history(x)[, c("Package", "date")]
...
bioc_n <- nrow(available.packages(
  contrib.url(paste0("https://bioconductor.org/packages/", bioc_v, "/bioc"))
))
```

There were `r cran_n` R packages available on [CRAN](https://cran.r-project.org/web/packages/) in `r params$year`, and that number only continues to grow over time. That’s not to mention the `r bioc_n` R packages available on BioConductor, and the unknown amount of packages only available on R-universe or GitHub.
...
Most R packages available on CRAN are actively maintained and updated. It's good practice to keep up with these updates so you can get the latest features, improvements, and bug fixes.
